...
# Prefer mamba when available because it dramatically reduces SAT solving time
# compared to the classic conda solver.  Falling back to conda preserves
# compatibility for users that do not have mamba installed.
...
if ! command -v "$SOLVER" >/dev/null 2>&1; then
  echo "[ERROR] Unable to locate solver '$SOLVER'. Install conda or mamba." >&2
  exit 1
fi
...
"$SOLVER" install -y pytorch torchvision torchaudio pytorch-cuda=11.8 -c pytorch -c nvidia

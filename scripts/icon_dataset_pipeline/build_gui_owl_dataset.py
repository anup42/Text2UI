...
2. Overlay bounding boxes + IDs and run the GUI-Owl-7B model to name each icon.
3. Merge detections with GUI-Owl outputs to produce YOLO-style label files and,
...
DEFAULT_PROMPT = (
    ...
    "with a numeric ID printed in green text. Produce labels separated by a single space without any newline using the exact format 'ID:name' e.g '1:delete 2:close'. "
...
class GenerationConfig:
    model_name: str = "mPLUG/GUI-Owl-7B"
...
def _parse_gui_owl_output(text: str) -> Dict[int, str]:
    labels: Dict[int, str] = {}
    pattern = re.compile(
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    gui_owl_output_dir = output_root / "intermediate" / "gui_owl_raw"
    labels_dir = output_root / "labels"
    viz_dir = output_root / "visualizations"

    overlay_dir.mkdir(parents=True, exist_ok=True)
    gui_owl_output_dir.mkdir(parents=True, exist_ok=True)
    ...
    _configure_attention(args.gui_owl_attn_backend)

    gui_owl_config = GenerationConfig(
        model_name=args.gui_owl_model,
        dtype=args.gui_owl_dtype,
        max_new_tokens=max(1, args.gui_owl_max_new_tokens),
        temperature=args.gui_owl_temperature,
        top_p=args.gui_owl_top_p,
        device_map=args.gui_owl_device_map,
        use_fast_processor=args.gui_owl_use_fast_processor,
        load_in_8bit=args.gui_owl_load_in_8bit,
        load_in_4bit=args.gui_owl_load_in_4bit,
        use_cache=args.gui_owl_use_cache,
    )
    print(
        file=sys.stderr,
    )

    max_memory_override = args.gui_owl_max_memory or _auto_max_memory_string()
    ...
    def handle_result(overlay_path: Path, raw_text: str) -> None:
        ...
        id_to_label = _parse_gui_owl_output(raw_text or "")
        label_file = labels_dir / f"{image_path.stem}.json"
    ...
    gui_owl_outputs = generate_icon_names(
        image_paths=overlay_paths,
        config=gui_owl_config,
        prompt=args.gui_owl_prompt or DEFAULT_PROMPT,
        batch_size=max(1, args.gui_owl_batch_size),
        output_dir=gui_owl_output_dir,
        quiet=args.quiet,
        max_edge=args.gui_owl_max_edge,
        attn_backend=args.gui_owl_attn_backend,
        max_memory=max_memory_override,
        offload_dir=Path(args.gui_owl_offload_dir) if args.gui_owl_offload_dir else None,
        on_result=handle_result,
    )

    remaining = [path for path in overlay_info if path not in processed_overlays]
    for overlay_path in remaining:
        raw_text = gui_owl_outputs.get(overlay_path, "")
        handle_result(overlay_path, raw_text)

def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Detect icons and build JSON label files using GUI-Owl-7B.")
    ...
    parser.add_argument("--quiet", action="store_true", help="Suppress GUI-Owl progress bar output.")

    parser.add_argument("--gui-owl-model", default="mPLUG/GUI-Owl-7B")
    parser.add_argument("--gui-owl-dtype", default="float16")
    parser.add_argument("--gui-owl-batch-size", type=int, default=1)
    parser.add_argument("--gui-owl-prompt", default=None, help="Override the default GUI-Owl prompt.")
    parser.add_argument(
        "--gui-owl-max-new-tokens",
        type=int,
        default=160,
        help="Maximum number of tokens the model may generate per image response.",
    )
    parser.add_argument("--gui-owl-temperature", type=float, default=0.1)
    parser.add_argument("--gui-owl-top-p", type=float, default=0.9)
    parser.add_argument("--gui-owl-device-map", default="auto")
    parser.add_argument("--gui-owl-use-fast-processor", action="store_true")
    parser.add_argument("--gui-owl-load-in-8bit", action="store_true")
    parser.add_argument("--gui-owl-load-in-4bit", action="store_true")
    parser.add_argument("--gui-owl-use-cache", action="store_true")
    parser.add_argument("--gui-owl-attn-backend", default="sdpa")
    parser.add_argument("--gui-owl-max-edge", type=int, default=1024)
    parser.add_argument("--gui-owl-max-memory", default=None, help='Per-device max memory map (e.g. "31GiB").')
    parser.add_argument("--gui-owl-offload-dir", default=None, help="Folder for model offload files.")
    return parser
